...
![image-20200804180905603](JAVA 线上故障排查完整套路/image-20200804180905603.png)

然后将占用最高的 pid 转换为 16 进制`printf '%x\n' pid`得到 nid

![image-20200804181422108](JAVA 线上故障排查完整套路/image-20200804181422108.png)

接着直接在 jstack 中找到相应的堆栈信息 `jstack pid |grep 'nid' -C5 –color`

![image-20200804181522141](JAVA 线上故障排查完整套路/image-20200804181522141.png)
...
![image-20200804191935405](JAVA 线上故障排查完整套路/image-20200804191935405.png)
...
![image-20200804193048022](JAVA 线上故障排查完整套路/image-20200804193048022.png)

## 上下文切换

针对频繁上下文问题，我们可以使用vmstat命令来进行查看

![image-20200804193301771](JAVA 线上故障排查完整套路/image-20200804193301771.png)

`cs(context switch)`一列则代表了上下文切换的次数。

如果我们希望对特定的 pid 进行监控那么可以使用 `pidstat -w pid`命令，cswch 和 nvcswch 表示自愿及非自愿切换。

![image-20200804193452996](JAVA 线上故障排查完整套路/image-20200804193452996.png)
...
![image-20200804193644715](JAVA 线上故障排查完整套路/image-20200804193644715.png)

更多时候，磁盘问题还是性能上的问题。我们可以通过 ` iostatiostat -d -k -x`来进行分析

![image-20200804193728463](JAVA 线上故障排查完整套路/image-20200804193728463.png)
...
![image-20200804193819517](JAVA 线上故障排查完整套路/image-20200804193819517.png)

不过这边拿到的是 tid，我们要转换成 pid，可以通过 readlink 来找到 `pidreadlink -f /proc/*/task/tid/../..`。

![image-20200804193908648](JAVA 线上故障排查完整套路/image-20200804193908648.png)

找到 pid 之后就可以看这个进程具体的读写情况`cat /proc/pid/io`

![image-20200804193947219](JAVA 线上故障排查完整套路/image-20200804193947219.png)
...
![image-20200804194053104](JAVA 线上故障排查完整套路/image-20200804194053104.png)
...
![image-20200804194156123](JAVA 线上故障排查完整套路/image-20200804194156123.png)
...
![image-20200804194432512](JAVA 线上故障排查完整套路/image-20200804194432512.png)
...
![image-20200804194517013](JAVA 线上故障排查完整套路/image-20200804194517013.png)
...
![image-20200804194622658](JAVA 线上故障排查完整套路/image-20200804194622658.png)

或者直接通过查看 `/proc/pid/task`的数量即为线程数量。

![image-20200804194651526](JAVA 线上故障排查完整套路/image-20200804194651526.png)
...
![image-20200804194750557](JAVA 线上故障排查完整套路/image-20200804194750557.png)

我们如果确定有可疑的内存端，需要通过 gdb 来分析 `gdb --batch --pid {pid} -ex "dump memory filename.dump {内存起始地址} {内存起始地址+内存块大小}"`

![image-20200804194845740](JAVA 线上故障排查完整套路/image-20200804194845740.png)
...
![image-20200804194946374](JAVA 线上故障排查完整套路/image-20200804194946374.png)

然后等放一段时间后再去看看内存增长的情况，通过 `jcmd pid VM.native_memory detail.diff(summary.diff)`做一下 summary 或者 detail 级别的 diff。

![image-20200804195020330](JAVA 线上故障排查完整套路/image-20200804195020330.png)

![image-20200804195054070](JAVA 线上故障排查完整套路/image-20200804195054070.png)
...
![image-20200804195125137](JAVA 线上故障排查完整套路/image-20200804195125137.png)

此外在系统层面，我们还可以使用 strace 命令来监控内存分配  `strace -f -e "brk,mmap,munmap" -p pid`

这边内存分配信息主要包括了 pid 和内存地址。

![image-20200804195156026](JAVA 线上故障排查完整套路/image-20200804195156026.png)
...
![image-20200804195242944](JAVA 线上故障排查完整套路/image-20200804195242944.png)
...
![image-20200804195408401](JAVA 线上故障排查完整套路/image-20200804195408401.png)
...
![image-20200804195445055](JAVA 线上故障排查完整套路/image-20200804195445055.png)

如上图所示，overflowed 表示全连接队列溢出的次数，sockets dropped 表示半连接队列溢出的次数。

### ss 命令，执行 ss -lnt

![image-20200804195507375](JAVA 线上故障排查完整套路/image-20200804195507375.png)
...
![image-20200804195659388](JAVA 线上故障排查完整套路/image-20200804195659388.png)

接下来我们通过 wireshark 打开抓到的包，可能就能看到如下图所示，红色的就表示 RST 包了。

![image-20200804195730644](JAVA 线上故障排查完整套路/image-20200804195730644.png)
...
![image-20200804195825604](JAVA 线上故障排查完整套路/image-20200804195825604.png)
