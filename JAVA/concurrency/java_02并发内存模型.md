...
A happens B：JMM 仅要求前一个操作A的操作结果对后一个操作B可见，且前一个操作A按顺序排在后一个操作B之前。并不要求前一个操作A必须在后一个操作B之前执行。
...
**JMM 非顺序一致性内存模型，所以顺序一致性内存模型的相关特效，对JMM并不适用。**
...
![image-20201031191749372](java_02并发内存模型/image-20201031191749372.png)
